...
class FkIk_UI:
	...
	def initializeTab(self, tabHeight, tabWidth):
		...
		pm.separator(h=5)
		pm.text('Release 1.7                  Monika Gelbmann                     05/2018')
		pm.separator(h=5)
	...
	def matchIkFkWin(self, tofk=1):
		...
		_logger.debug('rotOffset Z is %s' % rotOffsetZ)
		if tofk == 1:
...
def fkikMatch(fkwrist, fkellbow, fkshldr, ikwrist, ikpv, switchCtrl, switchAttr, switch0isfk=1, switchAttrRange=1, rotOffset=[0,0,0]):
	...
	fkwrist_key, fkellbow_key, fkshldr_key = pm.keyframe(fkwrist, q=1, t=(pm.currentTime(),pm.currentTime())),\
											 pm.keyframe(fkellbow, q=1, t=(pm.currentTime(),pm.currentTime())),\
											 pm.keyframe(fkshldr, q=1, t=(pm.currentTime(),pm.currentTime()))
	...
	_logger.debug('Start Cleanup FK to IK.')
	# clean up
	pm.delete(pm.ls(ikwristDup))
	pm.delete(snapGrp)
	#pm.delete(pvLoc)
	...
	_logger.debug('Finish Cleanup FK to IK.')
...
def ikfkMatch(fkwrist, fkellbow, fkshldr, ikwrist, ikpv, switchCtrl, switchAttr, switch0isfk=1, switchAttrRange=1, rotOffset=[0,0,0]):
	...
	#test malcoolm
	fkwrist="malcolm:ctlHandRt"
	fkellbow="malcolm:ctlArmLoRt"
	fkshldr="malcolm:ctlArmUpRt"
	ikwrist="malcolm:ctlArmIkRt"
	ikpv="malcolm:ctlArmPoleVectorRotRt"
	switchCtrl ="malcolm:ctlArmIkRt"
	switchAttr="Ik"
	switch0isfk=1
	switchAttrRange=1
	rotOffset=[0,0,0]
	...
	_logger.debug('enter ikfkMatch. Switch is %s' % switch)

	ikwrist, ikpv = pm.PyNode(ikwrist), pm.PyNode(ikpv)
	if pm.objExists('snapGrp'): pm.delete('snapGrp')
	snapGrp = pm.createNode('transform', name='snapGrp')

	# store if keyframe on ik attribute or not:
	ikwrist_key, ikpv_key = pm.keyframe(ikwrist, q=1, t=(pm.currentTime(), pm.currentTime())),\
											 pm.keyframe(ikpv, q=1, t=(pm.currentTime(), pm.currentTime()))
	ikwrist_matrix, ikpv_matrix = ikwrist.getMatrix(), ikpv.getMatrix()
	...
	# consider too small jointOrientation causeing ikFlip
	if ((fkellbow_ikSnap.jointOrientY.get() < 1) & (fkellbow_ikSnap.jointOrientX.get() < 1) & (
			fkellbow_ikSnap.jointOrientZ.get() < 1)):
		_logger.info("Small joint Orientation. Need to set Prefered Angle")
		fkellbow_ikSnap.ry.set(12)
		pm.joint(e=1, spa=1, ch=1)
		fkellbow_ikSnap.ry.set(0)
	...
	# snap ik writst to fk, snap pole_ikSnap to pole
	snap(fkwrist, ikwrist)
	snap(ikpv, pole_ikSnap)
	pm.poleVectorConstraint(pole_ikSnap, ikHandle_ikSnap[0])
	pm.parentConstraint(ikwrist, ikHandle_ikSnap[0], mo=1)
	print 'iksnap 0 is %s'%ikHandle_ikSnap[0]
	...
	# reset ik to original position
	ikwrist.setMatrix(ikwrist_matrix)
	...
	print 'ikwrist is %s'%ikwrist
	print 'ikHandle_ikSnap is %s'%ikHandle_ikSnap[0]
	#snap(ikwrist, ikHandle_ikSnap[0])
	...
	if (debug == False): pm.delete(clist)
	...
	# clean up
	if (debug == False): pm.delete(snapGrp)
	...
	if switch0isfk == 0:
		pm.setAttr(switch, 1)
		if switchAttrRange == 10: pm.setAttr(switch, 10)
	else: pm.setAttr(switch, 0)
...
def snap(master=None, slave=None, pos=1, rot=1):
	...
	for slave in slaves:
		...
		print slaveDup
		pm.delete(pm.ls(ptC))
		pm.delete(pm.ls(slaveDup))
		_logger.debug('Done Slave %s'%slave)
	try: pm.select(lastSel)
	except: pass
	_logger.debug("Returning snap")
