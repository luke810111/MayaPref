...
'''
added functions for skinningtool:
- setDrawInFront
'''
...
class CurvePrim(Primitive):
    ...
    def setDrawInFront(self, inBool = False):
        self.drawInFront = inBool
    ...
    def draw(self, view, renderer):
        ...
        glFT.glEnd()
        if self.drawInFront:
            glFT.glEnable(omr.MGL_DEPTH_TEST)
        glFT.glPopAttrib()
        view.endGL()
...
class VectorPrim(Primitive):
    ...
    def setDrawInFront(self, inBool = False):
        self.body.setDrawInFront(inBool)
        self.head.setDrawInFront(inBool)
